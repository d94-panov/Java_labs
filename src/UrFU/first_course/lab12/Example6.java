...
public class Example6 {
    public static void main(String[] args) {
        try {
            File folder = new File("C:\\Users\\dpabe\\IdeaProjects\\untitled\\src\\UrFU\\first_course\\lab12\\My"); // Создание папки «My» на диске, если она не существует
            if (!folder.exists())
                folder.mkdir();

            File f1 = new File("C:\\Users\\dpabe\\IdeaProjects\\untitled\\src\\UrFU\\first_course\\lab12\\My\\num1Mart.txt"); // Создание файла в папке
            f1.createNewFile();
...
// Поиск заданного числа в файле и определение его номера (номеров)
            ...
            for (int k = 0; k < count; k++) { // k – номер просмотра
                for (int i = 0; i < count - k - 1; i++) { // i – номер числа
                    ...
                    int number1 = rf.readInt(); // чтение i-того
                    int number2 = rf.readInt(); // и (i+1)-го чисел в переменные
